#!/bin/bash
if [[ $# -lt 3 ]]; then
  echo "Illegal arguments: init_docker_registry_client_by_insecure.sh registry_ip registry_name os"
  echo "e.g. $ /bin/bash init_docker_registry_client_by_insecure.sh 10.1.62.214 dock.cbd.com:80 linux"
  exit 128
fi
